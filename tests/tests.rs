use libwallet::{self, sr25519::Pair, Result, SimpleVault};
...
async fn test_account_derivation() {
    ...
    let key = get_address_from_mnemonic_and_derivation_path(MNEMONIC, derivation_path)
        .await
        .unwrap();
    assert_eq!(key, "5FRjccB7s9fbMu4pwQhYng2quQnKYkCHXRUCMBRwL7Pzj8FX");
}

async fn get_address_from_mnemonic_and_derivation_path(
    mnemonic: &str,
    derivation_path: &str,
) -> Result<String> {
    let vault = SimpleVault::<Pair>::from_phrase(mnemonic);
    ...
    subaccount.map(|a| a.public().to_string())
}
...
async fn test_serialize_root_account() {
    ...
    assert_eq!(
        account_json,
        "{\"account_type\":\"root\",\"network\":{\"Substrate\":42}}"
    );
}
