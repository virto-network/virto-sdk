...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // ```
    //
    // Here we demonstrate with async-std's TcpStream to show it works with raw sockets:
    ...
    let transport = HttpTransport::new("https://kreivo.io", |url: &str, body: Vec<u8>| {
        ...
        async move {
            ...
            // Here we shell out to the WS backend just to prove the transport layer works.
            let _ = (url, body);
